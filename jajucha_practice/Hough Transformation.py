...
class Planning(BasePlanning):
    ...
    def process(self, t, frontImage, rearImage, frontLidar, rearLidar):
        ...
        thres1 = 50
        thres2 = 30
        dst = canny[224:640,:].copy()
        converted = frontImage.copy()
        lines = cv2.HoughLinesP(dst,rho = 1,theta = np.pi/180,threshold = 50,minLineLength = thres1,maxLineGap = thres2)
        for i in range(len(lines)):
            for x1,y1,x2,y2 in lines[i]:
                cv2.line(converted,(x1,y1+224),(x2,y2+224),(255,0,0),1)

        cv2.imshow('img1',converted)
        
        # 차선 정보 파악
        V, L, R = self.gridFront(converted, cols=7, rows=3)
        ...
        #         cv2.line(frontImage,(x1,y1),(x2,y2),(0,0,255),2)

        # cv2.imshow('img',frontImage)
        # thres1 = 50
        # thres2 = 30
        # dst = canny[225:640,:].copy()
        # converted = frontImage[225:640,:].copy()
        # lines = cv2.HoughLinesP(dst,rho = 1,theta = np.pi/180,threshold = 50,minLineLength = thres1,maxLineGap = thres2)
        # for i in range(len(lines)):
        #     for x1,y1,x2,y2 in lines[i]:
        #         cv2.line(converted,(x1,y1),(x2,y2),(255,0,0),2)
