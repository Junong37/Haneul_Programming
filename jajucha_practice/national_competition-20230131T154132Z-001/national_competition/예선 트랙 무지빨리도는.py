    ...
    def process(self, t, frontImage, rearImage, frontLidar, rearLidar):
        ...
        #우회전
            ...
            if L.count("없음") < R.count("없음") and V[Idx_shift_L] > V[Idx_L]:
                ...
                steer = int (90-angle_L) +standard_steer + 20 
                status = "기울기 커브"
                if V[10] < 50: 
       ...
       # (2) T자
        if 18 >= L.count("없음") >= 12 and 18 >= R.count("없음") >= 12:     #만약 딜레이가 심한 환경이라면 18> 없애주기
            steer = 50 + 500//bunmo + standard_steer
            velocity = 40
            status = "T자"
