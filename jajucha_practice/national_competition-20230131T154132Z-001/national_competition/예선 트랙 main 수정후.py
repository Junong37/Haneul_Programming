       ...
       # (1) 1번 삼거리 
        if R.count("없음") >= 15 :
            ...
            if V.count(255) >= 4 and R.count("없음") <= 16: 
                ...
                status = "삼거리"
            elif (Idx_R >= 5 and  V[20] > 200): steer = 45 + 350 // bunmo +standard_steer
            elif Idx_R >= 15: steer = 45 + 350 //bunmo + standard_steer

       # (2) T자
        if L.count("없음") >= 10 and R.count("없음") >= 13:     #만약 딜레이가 심한 환경이라면 18> 없애주기 __ 정현이형 한테 보내기 위해 조건 없앴음(10/25)
            bunmo= V[20] // 10
            if 100 < V[20] < 190:
