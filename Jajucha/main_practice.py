...
class Planning(BasePlanning):
    ...
    def process(self, t, frontImage, rearImage, frontLidar, rearLidar):
        ...
        # 차선 유지
        if L[1] != 325 and R[2] - L[2] <= -30 and V[1] - V[5] <= 75:
            print("left")
            steer = (R[2] - L[2]) * 0.4
        elif R[1] != 316 and R[2] - L[2] >= 30 and V[5] - V[1] <= 75:
            print("right")
            steer = (R[2] - L[2]) * 0.4

        # 회전
        if V[5] <= 80 and V[5] <= V[1] and V[2] >= V[4]:
            print("left turn")
            steer = (100 - V[5]) * -0.7
        elif V[1] <= 80 and V[5] >= V[1] and V[2] <= V[4]:
            print("right turn")
            steer = (100 - V[1]) * 0.7
